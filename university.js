...
export class university {
    ...
    builSections() {
        this.sections.push({ sectionId: 1, sectionName: 'Section A' }, { sectionId: 2, sectionName: 'Section B' }, { sectionId: 3, sectionName: 'Section C' });
    }
    async addStudent(studentId = 0) {
        if (studentId === 0) {
            ...
            this.students.push({ studentId: studentIdRan,
            ...
            let idToPass = studentId.split('-')[0].trim();
            this.students.splice(idx, 1, { studentId: idToPass,
    ...
    async startProg() {
        try {
            ...
            if (opt.option.includes("1")) {
                await this.addStudent(0);
                await this.getStudents();
                return false;
            }
            else if (opt.option.includes("2")) {
                //console.log('update called')
                let id = await this.selectStudent();
                //let d = await this.debugPrint(id);
                await this.addStudent(id);
                await this.getStudents();
                return false;
            }
            else if (opt.option.includes("3")) {
                await this.addTeacher(0);
                await this.getTeachers();
                return false;
            }
            else if (opt.option.includes("4")) {
                let t = await this.selectTeacher();
                await this.addTeacher(t);
                await this.getTeachers();
                return false;
            }
            else if (opt.option.includes("5")) {
                await this.getStudents();
                return false;
            }
            else if (opt.option.includes("6")) {
                await this.getTeachers();
                return false;
            }
            else if (opt.option.includes("7")) {
                await this.getStudentsByCourse();
                return false;
            }
            else if (opt.option.includes("8")) {
    ...
    async addTeacher(teacherId = 0) {
        ...
        if (teacherId === 0) {
            ...
            this.teachers.push({ teacherId: teacherIdRan,
            ...
            let idToPass = teacherId.split('-')[0].trim();
            this.teachers.splice(idx, 1, { teacherId: idToPass,
